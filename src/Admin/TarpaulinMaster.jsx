...
function TarpaulinMaster() {
  const [tarpaulins, setTarpaulins] = useState([]);
  ...
  // Fetch all Tarpaulins
  const fetchTarpaulins = async () => {
    try {
      setLoading(true);
      const response = await httpClient.get('/getAllTarpaulin');
      setTarpaulins(response.data);
    } catch (error) {
      console.error('Error fetching tarpaulins:', error);
      showErrorMessage('Error fetching tarpaulins');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!imageFile && !formData.tarpaulin_image) {
      showErrorMessage('Please select an image');
      return;
    }

    if (!formData.tarpaulin_title.trim()) {
      showErrorMessage('Please enter tarpaulin title');
      return;
    }

    if (!formData.tarpaulin_description.trim()) {
      showErrorMessage('Please enter tarpaulin description');
      return;
    }

    try {
      setLoading(true);
      let imageUrl = formData.tarpaulin_image;
      ...
      const tarpaulinData = {
        tarpaulin_image: imageUrl,
        tarpaulin_title: formData.tarpaulin_title.trim(),
        tarpaulin_description: formData.tarpaulin_description.trim(),
        status: formData.status,
      };

      console.log('Sending tarpaulin data:', tarpaulinData);

      if (editingId) {
        // Update existing tarpaulin
        await httpClient.put('/update_Tarpaulin', { ...tarpaulinData, _id: editingId });
        showSuccessMessage('Tarpaulin updated successfully!');
      } else {
        // Create new tarpaulin
        await httpClient.post('/create_Tarpaulin', tarpaulinData);
        showSuccessMessage('Tarpaulin created successfully!');
      }

      resetForm();
      fetchTarpaulins();
    } catch (error) {
      console.error('Error saving tarpaulin:', error);
      showErrorMessage('Error saving tarpaulin: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Handle edit
  const handleEdit = (tarpaulin) => {
    setFormData({
      tarpaulin_image: tarpaulin.tarpaulin_image,
      tarpaulin_title: tarpaulin.tarpaulin_title,
      tarpaulin_description: tarpaulin.tarpaulin_description,
      status: tarpaulin.status
    });
    setImagePreview(tarpaulin.tarpaulin_image);
    setEditingId(tarpaulin._id);
    setImageFile(null);
    setShowForm(true);
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      try {
        await httpClient.delete(`/delete_Tarpaulin/${id}`);
        showSuccessMessage('Tarpaulin deleted successfully!');
        fetchTarpaulins();
      } catch (error) {
        console.error('Error deleting tarpaulin:', error);
        showErrorMessage('Error deleting tarpaulin');
      }
    }
  };
      ...
      modal.innerHTML = `
        <div class="bg-white rounded-xl max-w-sm w-full p-4 sm:p-6 mx-2">
          <div class="text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-800 mb-2">Delete Tarpaulin</h3>
            <p class="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base">Are you sure you want to delete this tarpaulin? This action cannot be undone.</p>
    ...
    <div className="p-3 sm:p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3 sm:gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800">Tarpaulin Master</h1>
            <p className="text-gray-600 mt-1 text-sm sm:text-base">Manage your tarpaulin products and descriptions</p>
          </div>
          <button
        ...
        {/* Tarpaulin Form Modal */}
        {showForm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-50">
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center p-4 sm:p-6 border-b">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-800">
                  {editingId ? 'Edit Tarpaulin' : 'Add New Tarpaulin'}
                </h2>
                <button
                  onClick={resetForm}
              ...
              <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
                  ...
                  <div className="flex flex-col items-center space-y-3 sm:space-y-4">
                    <div className="w-full max-w-xs mx-auto">
                      <div className={`border-2 border-dashed rounded-lg p-3 sm:p-4 text-center transition-colors duration-200 ${
                        imagePreview || formData.tarpaulin_image ? 'border-green-300 bg-green-50' : 'border-gray-300 hover:border-green-400'
                      }`}>
                        {imagePreview || formData.tarpaulin_image ? (
                          <img
                            src={imagePreview || formData.tarpaulin_image}
                            alt="Tarpaulin preview"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="tarpaulin_title"
                    value={formData.tarpaulin_title}
                    ...
                    placeholder="Enter tarpaulin title (e.g., HDPE Tarpaulin Sheet)"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    name="tarpaulin_description"
                    value={formData.tarpaulin_description}
                    onChange={handleInputChange}
                    rows={3}
                    ...
                    placeholder="Enter tarpaulin description and features"
                    required
                  />
                </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[600px]">
                ...
                <tbody className="divide-y divide-gray-200">
                  {tarpaulins.map((tarpaulin, index) => (
                    <tr key={tarpaulin._id} className="hover:bg-gray-50 transition-colors duration-150">
                      ...
                      <td className="px-3 sm:px-4 py-3 sm:py-4">
                        <div className="flex items-center justify-center sm:justify-start">
                          <div className="w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center bg-gray-100 rounded-lg border">
                            <img
                              src={tarpaulin.tarpaulin_image}
                              alt={`Tarpaulin ${tarpaulin.tarpaulin_title}`}
                      ...
                      <td className="px-3 sm:px-4 py-3 sm:py-4">
                        <div className="max-w-[150px] sm:max-w-xs">
                          <p className="text-sm font-medium text-gray-900 truncate" title={tarpaulin.tarpaulin_title}>
                            {tarpaulin.tarpaulin_title}
                          </p>
                        </div>
                      </td>
                      <td className="px-3 sm:px-4 py-3 sm:py-4">
                        <div className="max-w-[200px]">
                          <p className="text-sm text-gray-600 line-clamp-2" title={tarpaulin.tarpaulin_description}>
                            {tarpaulin.tarpaulin_description}
                          </p>
                        </div>
                      </td>
                      <td className="px-3 sm:px-4 py-3 sm:py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          tarpaulin.status === 'active' 
                            ? 'bg-green-100 text-green-800 border border-green-200' 
                            : 'bg-red-100 text-red-800 border border-red-200'
                        }`}>
                          {tarpaulin.status.charAt(0).toUpperCase() + tarpaulin.status.slice(1)}
                        </span>
                      </td>
                      <td className="px-3 sm:px-4 py-3 sm:py-4 whitespace-nowrap">
                        <div className="flex items-center gap-1 sm:gap-2">
                          <button
                            onClick={() => handleEdit(tarpaulin)}
                          ...
                          <button
                            onClick={() => handleDelete(tarpaulin._id)}
              ...
              {tarpaulins.length === 0 && (
                <div className="text-center py-8 sm:py-12 text-gray-500">
                  ...
                  <p className="text-base sm:text-lg font-medium text-gray-600 mb-2">No tarpaulins found</p>
                  <p className="text-gray-500 mb-4 text-sm sm:text-base">Get started by creating your first tarpaulin</p>
                  <button
                    onClick={() => setShowForm(true)}
...
export default TarpaulinMaster;