...
function MainProduct() {
  ...
  const { category, subProduct } = location.state || {};

  useEffect(() => {
    const fetchProducts = async () => {
      if (!subProduct) {
        setLoading(false);
        setError('No sub-product selected');
        return;
      }
      ...
      try {
        ...
        if (response.data && Array.isArray(response.data)) {
          // Filter products by subcategory ID and only active status
          const filtered = response.data.filter(
            product => 
              product.Subcategory?._id === subProduct._id &&
  ...
  useEffect(() => {
    if (searchTerm.trim() === '') {
      ...
      const filtered = products.filter(product =>
        ...
        product.GSM_Thickness?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">
              {subProduct?.gsm_name} <span className="text-red-600">Products</span>
            </h1>
            <div className="w-24 h-1 bg-orange-500 mx-auto mb-4"></div>
            <p className="text-gray-600 max-w-2xl mx-auto text-lg">
              Explore our premium range of {subProduct?.gsm_name} products under {category?.name}
            </p>
          </div>
        </div>
        ...
        <div className="mb-8 max-w-md mx-auto">
          <div className="relative">
            <input
              type="text"
              placeholder={`Search ${subProduct?.gsm_name} products...`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
              {filteredProducts.map((product, index) => (
                <div
                  ...
                  className="group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden border-2 border-orange-500 cursor-pointer"
                  onClick={() => handleProductClick(product)}
                >
                  {/* Product Image Container */}
                  <div className="relative overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 p-6">
                    <div className="flex items-center justify-center h-48">
                      {/* FIXED: Use product.image instead of product.images[0] */}
                      {product.image ? (
                        <>
                          <img
                            src={product.image}
                          ...
                          <div 
                            className="hidden items-center justify-center h-40 w-40 bg-gray-200 rounded-lg"
                            style={{ display: 'none' }}
                          >
                  ...
                  <div className="p-6">
                    <div className="mb-3">
                      ...
                      <h3 className="text-xl font-bold text-gray-900 mb-4 leading-tight text-center group-hover:text-red-600 transition-colors duration-300">
                        {product.ProductName}
                      </h3>
                      ...
                      <div className="mt-6 flex justify-center">
                        <button 
                          className="bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white font-bold py-3 px-8 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl flex items-center space-x-2"
                          onClick={(e) => {
                            e.stopPropagation();
                ...
                <div className="bg-white rounded-2xl shadow-lg p-8 max-w-2xl mx-auto">
                  ...
                  <p className="text-gray-600 mb-6">
                    {searchTerm 
                      ...
                      : `There are no products available for ${subProduct?.gsm_name} at the moment.`
                    }
                  </p>
                  {searchTerm ? (
                    <button 
